    ...
    const test = async () => {
      ...
      try {
        ...
        err.message.should.match(
          /A partition manager cannot be started multiple times/gi
        );
      } finally {
        await host.stop();
        should.equal(
          host["_context"]["partitionManager"]["_isCancelRequested"],
          true
        );
      }
    };
    ...
    host
      ...
      .then(() => {
        return Promise.reject(
          new Error("This statement should not have executed.")
        );
      })
      .catch(err => {
        debug(">>>>>>> %s", err.action);
