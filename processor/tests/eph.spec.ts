  ...
  describe("single", function() {
    it("should checkpoint messages in order", function(done) {
      const test = async () => {
        ...
        const ehc = EventHubClient.createFromConnectionString(
          ehConnString!,
          hubName!
        );
        ...
        const onMessage = async (
          ...
          if (num % 10 === 0) {
            ...
            try {
              ...
              debug(
                ">>>>>>> An error occurred while checkpointing msg number %d: %O",
                num,
                err
              );
            }
        ...
        while (!doneCheckpointing) {
          debug(
            "Not done checkpointing -> %s, sleeping for 10 more seconds.",
            doneCheckpointing
          );
          await delay(10000);
        }
        ...
        await delay(10000);
        const stringContent = await host["_context"].blobReferenceByPartition[
          "0"
        ].getContent();
              ...
              const onMessage: OnReceivedMessage = (
                ...
                if (data.properties!.message_id === msgId) {
                  ...
                  context
                    ...
                    .then(content => {
                      debug(
                        ">>>> Seen expected message. New lease contents: %s",
                        content
                      );
      ...
      const test = async () => {
        ...
        const onMessage: OnReceivedMessage = async (
          ...
          if (
            ...
            firstSend[partitionId].properties!.message_id
          ) {
            debug(">>>> Checkpointing the received message...");
            await context.checkpoint();
            count++;
        ...
        while (count < ids.length) {
          await delay(10000);
          debug(
            ">>>> number of partitionIds: %d, count: %d",
            ids.length,
            count
          );
        }
        ...
        const onMessage2: OnReceivedMessage = async (
          ...
          if (
            data.properties!.message_id ===
            secondSend[partitionId].properties!.message_id
          ) {
            debug(">>>> Checkpointing the received message...");
            await context.checkpoint();
            count2++;
        ...
        if (count2 > ids.length) {
          throw new Error(
            "We received more messages than we were expecting..."
          );
        }
      };
      test()
  ...
  describe("multiple", function() {
    it("should be able to run multiple eph successfully.", function(done) {
      const test = async () => {
        ...
        const getReceivingFromPartitionsForAllEph = (): Dictionary<
          string[]
        > => {
          const receivingPartitionsByHost: Dictionary<string[]> = {};
          for (const hostName in hostByName) {
            receivingPartitionsByHost[hostName] =
        ...
        for (let i = 0; i < ids.length; i++) {
          ...
          const onMessage: OnReceivedMessage = (
            ...
            should.equal(
              sendDataByPartition[context.partitionId].body,
              data.body
            );
          };
